...
public class ModelMapper {
    ...
    static {
        ...
        mapper.setCustomMapper(TLUsers.class, yayNode -> {
            if (! (yayNode instanceof YayObject users)) return new TLUsers(Map.of());
            return new TLUsers(users.value().values().stream().map(userCommonMapper).map(LightUser.class::cast).collect(Collectors.toMap(LightUser::id, Function.identity())));
        });
    ...
    public record Timeline(List<TLEntry> entries, TLUsers users) {
        Stream<TimelineEntry> toTimelineEntries() {
            return entries.stream().map(entry -> entry.toTimelineEntry(users.users())).filter(Objects::nonNull);
        }
    }

    public record TLEntry(String type, Map<String, Object> data, ZonedDateTime date) {
    ...
    public record TLUsers(Map<String, LightUser> users) {}
