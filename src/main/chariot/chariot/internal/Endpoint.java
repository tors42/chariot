...
public sealed interface Endpoint<T> {
    ...
    static record RoundConvert(String id, String slug, String name, ZonedDateTime createdAt, boolean startsAfterPrevious, Opt<ZonedDateTime> startsAt, Opt<ZonedDateTime> finishedAt, boolean ongoing, boolean finished, java.net.URI url, Integer delay) {
        MyRound.Round toRound() {
            return new MyRound.Round(id, slug, name, createdAt, startsAfterPrevious, startsAt, finishedAt, ongoing, finished, url, delay == null ? Duration.ZERO : Duration.ofSeconds(delay));
        }
    }
